...
In practice, the grid structure remains unchanged during normal operation, while the grid state is continually changing over time. For example, an important aspect of grid operation is \define{load profiling}: examining and predicting the total load connected to a node, as a function of time. As a result of changing loads, the flow of power in a grid is constantly changing.
...
\begin{align*}
    \mathsf{I=VY}
\end{align*}
This allows us to define the admittance between two unconnected nodes as $0$. (\ie no current is induced by a potential difference.) \todelete{We now construct the \emph{admittance matrix}\index{admittance!matrix} $\mat{Y} \in \mathbb{C}^{n \times n}$, where $\mel{Y}_{i,j}$ is the admittance between nodes $i$ and $j$. Note that $\mat{Y}$ is a symmetric matrix, with only $2m$ non-zero entries.} \todo{$\mat{Y}$ is not used very often and conflicts with the \emph{Nodal admittance matrix} (wiki)}
...
\towrite{TODO, I is directed along the graph edges}\todo{$\mat{I}$ could be left out of the grid state, since it is uniquely determined by $\mat{V}$ and the structure.}
...
\towrite{Written in this form, we see that when $i\mat{\eta}, \mat{C}$ and $\mat{V}$ are purely real, then $\mat{I}$ is purely imaginary. Physically, this means that line current is always $90\si{\degree}$ out of phase with node voltage. (Or it means that current is out of phase with voltage differences? Which are voltage angle differences?)}
...
\end{align}\todo{$\mat{M}$ should be $\pm i \mat{L}$}
...
Given an $(n,m)$-grid structure $((\mathcal{N},\mathcal{L}),\mat{C},\mat{\eta})$, a grid state $(\mat{S}, \mat{V}, \mat{I})$ is \emph{DC-valid}\index{valid!DC} if it satisfies the \emph{KVL-Ohm equality}:\todo{this is also approximated: $\mat{CV}$ is linearised.}
\begin{align}
...
\end{align*}
\end{proof}
\towrite{Appendix: give a direct formula for $\mat{L}$}
\towrite{compute $\mat{L}$ for the example networks}
\towrite{give an alternative interpretation of $\mat{L}$ or $\mat{L}$: discrete laplacian}
\towrite{discuss the kernel of $\mat{L}$: it should follow from graph theoretical arguments that the kernel is spanned by $(1,1,\dots,1)^T$, such that the nullity of $\mat{L}$ is $1$. This corresponds to the fact that phase angle is a relative quantity. (The state remains (DC-)valid when increasing the phase angle at each node by the same amount. (If $\mat{S}=\mat{L\theta}$ and $\mat{\theta}_0 \in \ker \mat{L}$, then $\mat{S}=\mat{L}(\mat{\theta}+\mat{\theta_0})$.))\\There are two paths to take after this point:\\1: Fix the phase angle of the slack bus (to $0$), which corresponds to taking the ${n-1 \times n-1}$ submatrix of $\mat{L}$, I think. \textbf{This is the approach of PyPSA, which might explain small differences in line flow.} \\2: Leave $\mat{L}$ as-is, and consider the \emph{Moore-Penrose pseudo-inverse} of $\mat{L}$. This is the approach of Nesti et al., which they attribute to 'distributive slack': not fixing a single slack bus.}
...
\begin{align*}
    ...
    &=
    KV_{op}^2 
    \sum_{\mathcal{L}_k=(a,b) \in \mathcal{L}} \abs{\mel{\eta}_k} (\mel{C\theta})_k^2
    \\
    &=
    KV_{op}^2 
    \norm{\mat{\eta}  \pointwise \mat{C\theta} \pointwise \mat{C\theta}}_{1} \qquad \text{(in $\ell^1$)}
    \\
    &\leq
    KV_{op}^2 
    \norm{\mat{\eta}}_{\infty}  \norm{\mat{C\theta} \pointwise \mat{C\theta}}_{1} \qquad \text{(Hölder's inequality)}
    \\
    &=
    KV_{op}^2 
    \norm{\mat{\eta}}_{\infty}  \norm{\mat{C\theta}}_2^2
...
\towrite{My DC approximated KVL-Ohm is incorrect, it should be: $\mat{I} = i\diag(i\mat{\eta})\mat{C\theta}V_{op}$.}

\begin{align*}
    \mat{\hat{f}} = -iV_{op}\mat{I}=-iV_{op}^2i\diag(i\mat{\eta})\mat{C\theta}
    = \diag(i\mat{\eta})\mat{C}\mat{L^+}\mat{p}.
\end{align*}
(We assumed a normalised profile, so $V_{op}=1$.)

If the line capacities\index{line capacity} are $\mat{W}=(W_1, \dots, W_m)^T \in \mathbb{R}^m$\todo{C already used}, then we can define the \emph{normalised line flow}\index{line flow!normalised} as:
$$\mat{f} = \diag(\mat{W})^{-1}\mat{\hat{f}}$$
and we find:
\begin{empheq}[box=\fbox]{gather}
    \mat{f}=\mat{F}\mat{p}\\[3mm]
    \text{where }\quad\mat{F}=\diag(\mat{W})^{-1}\diag(i\mat{\eta})\mat{C}\mat{L^+}
\end{empheq}

\emph{This is a linear transformation from a power injection vector $\mat{p}$ to the normalised line flow $\mat{f}$ that it induces.}\todo{$\mat{F}$ is not the unique transformation with this property (otherwise $\mat{F}$ would be square and non-singular). Does $\mat{F}$ minimise $\norm{\mat{f}}$?}
...
\[
\mat{p} \sim N(\mat{\mu_p}, \matSigma_\mat{p})
\]
where $\mat{\mu_p}$ is the mean power injection, which is the sum of deterministic generation and expected stochastic generation, minus the load. $\matSigma_\mat{p}$ is the \emph{power injection correlation matrix}. When power injection at different nodes is correlated (because of correlated weather), this matrix is non-diagonal.

Because $\mat{F}$ is a linear transformation, the vector of line flows $\mat{f}$ is also (multivariate) normally distributed, and its distribution is given by

\begin{equation}
\mat{f} \sim N(\mat{\mu_f}, \matSigma_\mat{f}), \quad \text{ where } \quad
\mat{\mu_f} = \mat{F}\mat{\mu_f} \quad \text{and} \quad 
\matSigma_\mat{f} = \mat{F}\matSigma_\mat{p}\mat{F}^T.
\end{equation}

\section{Applying the Large Devation Principle}
\towrite{We will study $\PROB(|\mel{f}_l|\geq 1)$ for each line $l$ using the LDP.}
\end{document}