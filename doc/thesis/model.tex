...
\towrite{Deze paragraaf is nog een beetje slordig!}
...
\towrite{Ik moet in deze paragraaf nog het `'dakje' op $\mathbf{f}$ zetten en $\mathbf{F}$ met $\diag(\mathbf{W})^{-1}$ schalen.}
...
\towrite{(In deze paragraaf moet geen dakje op $\mathbf{f}$.)}
...
\towrite{Als het netwerk niet meer verbonden is (power island) is ons model eigenlijk niet meer toepasbaar! We gebruiken nu de pseudoinverse, maar hier moet ik nog wat harder over nadenken.}
