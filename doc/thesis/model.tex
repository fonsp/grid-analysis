...
Given a directed graph with $n$ nodes and $m$ lines, we define the vertex-edge incidence matrix $\mat{K} \in \mathbb{R}^{n \times m}$ as:
\begin{align*}
    \mel{K}_{i,k} =
    \begin{cases}
...
Note that columns of $\mat{K}$ correspond to lines of the network.
\end{example}
