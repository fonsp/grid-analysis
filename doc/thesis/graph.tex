...
From (\ref{eq:inducedflow}), it follows that $\FRT$ is a linear map. In fact, when written in matrix form, we find a familiar result: the transposed edge-vertex incidence matrix of $G$!\todo{blub}
$$\FRT = \mat{C}^T$$
...
We will prove this equality by considering the two inclusions $\subseteq$ and $\supseteq$ separately. The first inclusion follows from the observation that $\FRT = \mat{C}^T$:
...
The vectors $\FRT(\mat{e}^1), \dots, \FRT(\mat{e}^m)$ are exactly the columns of the matrix $\FRT = \mat{C}^T$, which all have zero sum. 
\end{proof}
