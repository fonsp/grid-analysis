...
When $b=1$, we write $\mat{r}_A$, and we say that $\mat{r}_A$ is a \define{pillar} for $A$. Two planes are called \emph{parallel} if they do not intersect.
\end{definition}
\begin{remark}
...
Let $\mat{X} \, \sim \, \gaussdistr(\mat{\mu}, \mat{\Sigma})$ be a normally distributed $p$-vector with density function $f_{\mat{X}}$, and let $A\subseteq \mathbb{R}^p$ be a plane, given by a pillar $\mat{r}_A \in \mathbb{R}^p$. Then $\mat{X} \mid A$ \todo{hier bestaat vast notatie voor} is Gaussian distributed, and has mode\todo{$A$ is not an event, but $\mathbf{X}^{-1}(A)$ is.}
\begin{equation}
